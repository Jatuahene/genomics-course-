...
### select all columns except certain ones put a “-” in front of the variable to exclude it.
...
```
```{r, pipes}
### create a temporary data frame and use that as input to the next function, like this:
surveys2 <- filter(surveys, weight < 5)
surveys_sml <- select(surveys2, species_id, sex, weight)

### The last option, pipes, are a recent addition to R.
surveys %>%
  filter(weight < 5) %>%
...
### use the print() function at the end
surveys %>%
  filter(!is.na(weight)) %>%
  group_by(sex, species_id) %>%
  summarize(mean_weight = mean(weight)) %>%
...
```{r, counting}
###to count the number of rows of data for each sex, we would do:
surveys %>%
    count(sex) 
...
```{r,exportingdata}
### start by removing observations of animals for which weight and hindfoot_length are missing, or the sex has not been determined:
...
Application written in R [@RCoreTeam] using shiny framework [@Chang2015]. 
